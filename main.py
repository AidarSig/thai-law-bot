...
async def run_assistant(thread_id, assistant_id):
    try:
        # ОБНОВЛЕННАЯ ИНСТРУКЦИЯ (v7.8) - Без слов "база" и "файл"
        instructions = """
...
2. НЕ проси номер телефона первым.
...
АНТИ-ГАЛЛЮЦИНАЦИИ: Если ответа нет в материалах — НЕ ВЫДУМЫВАЙ. Сразу переводи на контакты операторов с фразой: "Этот вопрос требует индивидуального разбора юристом."

КОНТАКТНАЯ ИНФОРМАЦИЯ: Давай их только если вопрос сложный или требует юриста. Формат строго такой:
Контакты:
...
2. Если есть: ответь четко и по делу.
3. Если нет: направь на контакты.
...
@app.get("/")
def home():
    return {"status": "ThaiLawBot v7.8 (No 'Database' words)"}
